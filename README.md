...
> **Versi:** 2.1.0 | **Tanggal Rilis:** 24 Mei 2026 | **Lisensi:** MIT
...
### 2. Shortcut Nominal Bayar
- **UANG PAS** — Otomatis mengisi nominal = total pembelian
...
- Informasi lengkap: nama toko, tanggal, waktu, detail item, total, bayar, kembalian
...
- Detail per transaksi: waktu, item, total, bayar, kembalian, laba
...
- Statistik harian: Omzet, Laba Kotor, Transaksi, Item Terjual
- Jam real-time (timezone Asia/Jakarta)
- Versi aplikasi ditampilkan di header
...
│   ├── Sticky Header → Logo, Jam, Stats, Tombol Aksi
│   ├── Stats Row    → Ringkasan Dashboard
│   ├── Tab Nav       → Kasir (POS) | Laporan Penjualan
│   ├── Tab: POS      → Product Grid + Cart + Payment
│   ├── Tab: Laporan  → Tabel Riwayat + Summary
│   ├── Modal: CRUD   → Form Produk + Tabel Produk
│   ├── Modal: Struk  → Preview + Cetak
│   ├── Modal: Info   → Panduan + Shortcut + Changelog
│   └── Toast         → Notifikasi
└── <script>          → Business Logic (Data, Cart, Payment, CRUD, Report)
```
...
- **State Management** — Tiga variabel global (`PRODUCTS`, `CART`, `SALES`) sebagai single source of truth
...
3. **Bayar** — Klik shortcut nominal atau ketik manual. Tombol "UANG PAS" mengisi nominal = total belanja.
4. **Kembalian** — Sistem otomatis menghitung dan menampilkan kembalian. Jika uang kurang, ditampilkan peringatan.
5. **Konfirmasi** — Tekan "Bayar & Simpan". Transaksi tersimpan ke localStorage.
6. **Cetak Struk** — Modal struk otomatis muncul. Klik "Cetak Struk" untuk mengirim ke printer.
...
2. Pilih filter waktu (Hari Ini / 7 Hari / 30 Hari / Semua)
3. Lihat ringkasan di bagian bawah tabel
4. Klik "Export CSV" untuk download data
5. Klik ikon sampah di kolom Aksi untuk hapus transaksi individual
...
| `pos_sales_db` | Riwayat transaksi (id, timestamp, items, total, paid, change, profit) | JSON Array |
...
```json
{
  ...
  "items": [
    { "id": "string", "name": "string", "buyPrice": "number", "sellPrice": "number", "qty": "number" }
  ],
  "total": "number",
  "paid": "number",
  "change": "number",
...
- Estimasi: ~10.000 transaksi dapat disimpan sebelum mencapai batas
