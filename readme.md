...
![Version](https://img.shields.io/badge/version-3.0.0-blue) ![Language](https://img.shields.io/badge/language-HTML-orange) ![License](https://img.shields.io/badge/license-MIT-green) ![Status](https://img.shields.io/badge/status-Production%20Ready-brightgreen)

---

> **Versi:** 3.0.0 | **Tanggal Rilis:** 25 Mei 2026 | **Lisensi:** MIT | **Pencipta:** SAKA. OMNI - POS UMKM
...
**OMNI - POS UMKM** adalah aplikasi Point of Sale (POS) interaktif berbasis web yang dirancang untuk kebutuhan kasir modern di lingkungan UMKM Indonesia. Aplikasi ini berjalan sepenuhnya di browser tanpa memerlukan server backend, menjadikannya solusi ringan dan cepat untuk usaha kecil dan menengah seperti warung kopi, kedai makanan, toko kelontong, dan sejenisnya.

Dibangun dan diciptakan oleh **SAKA. OMNI - POS UMKM**, aplikasi ini menghadirkan pengalaman kasir digital yang profesional tanpa biaya langganan bulanan. Identitas toko dapat dikustomisasi melalui modul Profile Toko, sehingga setiap struk dan laporan yang dicetak mencerminkan branding usaha Anda sendiri.
...
- **Brand-Your-Own** — Profile Toko memungkinkan kustomisasi nama toko, WhatsApp, dan alamat yang otomatis tersinkronisasi ke struk dan laporan.
...
### 2. Profile Toko
- **Nama Toko** — Nama usaha yang tampil di header aplikasi, judul browser, struk, dan laporan
- **WhatsApp** — Nomor WhatsApp yang tercetak di struk untuk kemudahan pelanggan menghubungi toko
- **Alamat Jalan** — Alamat toko yang tercetak di struk pembelian
- **Preview Struk** — Pratinjau langsung tampilan struk saat mengisi profile
- **Persistent** — Data profile tersimpan di localStorage dan bertahan walau browser ditutup
- **Dinamis** — Perubahan profile langsung diterapkan ke seluruh aplikasi tanpa refresh
...
- **Identitas toko** — Nama toko, alamat, dan WhatsApp tercetak di bagian atas struk
...
- **Powered by SAKA. OMNI** — Footer pencipta tercetak di bagian bawah struk
...
- **Ringkasan Kasir Hari Ini** — Ditampilkan di tab laporan
...
- Export laporan ke file CSV (header menyertakan nama toko) untuk backup/analisis
...
- **Tab Tentang** — Informasi pencipta (SAKA. OMNI - POS UMKM), teknologi, dan versi aplikasi
...
│   ├── Sticky Header → Logo (dinamis dari Profile), Jam, Tx Count, Tombol Aksi
...
│   ├── Modal: Profile→ Form Profile Toko + Preview Struk
│   ├── Modal: CRUD   → Form Produk + Tabel Produk
│   ├── Modal: Diskon → Diskon Per Item (%, Rp)
│   ├── Modal: Struk  → Preview + Cetak (dengan identitas toko)
│   ├── Modal: Info   → Panduan + Shortcut + Changelog + Tentang
│   ├── Footer        → powered by SAKA. OMNI - POS UMKM
│   └── Toast         → Notifikasi
└── <script>          → Business Logic (Data, Profile, Cart, Discount, Payment, CRUD, Report)
```
...
- **State Management** — Empat variabel global (`PRODUCTS`, `CART`, `SALES`, `STORE_PROFILE`) + `CART_DISCOUNT` sebagai single source of truth
- **Discount Calculation** — Per-item discount dihitung terlebih dahulu, lalu global discount dihitung dari sisa subtotal
- **Dynamic Branding** — Profile toko mengganti nama di header, judul browser, tab tentang, struk, dan CSV secara dinamis
...
```
pos-umkm/
├── index.html      → Aplikasi utama (self-contained HTML)
├── readme3.md      → Dokumentasi lengkap (file ini)
├── SKILL.md        → Spesifikasi agentic skill untuk OMNI - POS UMKM
└── LICENSE         → Lisensi MIT
```
...
2. Akses via `http://domain.com/index.html`
...
1. **Setup Profile Toko** — Klik "Profile Toko" di header. Isi nama toko, WhatsApp, dan alamat. Data akan otomatis tersinkronisasi ke struk dan laporan.
2. **Pilih Produk** — Klik kartu produk di grid kiri. Produk akan otomatis masuk keranjang dengan qty = 1. Klik lagi untuk menambah qty.
3. **Atur Jumlah** — Gunakan tombol (+) dan (-) di keranjang. Qty minimum = 1. Jika dikurangi di bawah 1, item dihapus.
4. **Diskon (Opsional)** — Klik ikon tag pada item di keranjang untuk diskon per barang, atau atur diskon global di bagian Diskon.
5. **Bayar** — Klik shortcut nominal atau ketik manual. Tombol "UANG PAS" mengisi nominal = total belanja (setelah diskon).
6. **Kembalian** — Sistem otomatis menghitung dan menampilkan kembalian. Jika uang kurang, ditampilkan peringatan.
7. **Konfirmasi** — Tekan "Bayar & Simpan". Transaksi tersimpan ke localStorage.
8. **Cetak Struk** — Modal struk otomatis muncul dengan identitas toko. Klik "Cetak Struk" untuk mengirim ke printer.

### Profile Toko

1. Klik tombol "Profile Toko" di sticky header
2. Isi nama toko (wajib), WhatsApp, dan alamat jalan
3. Preview struk muncul secara real-time saat mengetik
4. Klik "Simpan Profile" untuk menyimpan
5. Perubahan langsung diterapkan ke:
   - Header aplikasi (nama toko mengganti "OMNI - POS UMKM")
   - Judul tab browser
   - Tab Tentang di modal Info
   - Struk pembelian (preview dan cetak)
   - Header file CSV laporan
...
5. Klik "Export CSV" untuk download data (header menyertakan nama toko)
...
| `pos_store_profile` | Profile toko (name, whatsapp, address) | JSON Object |
...
```json
{
  ...
  "whatsapp": "string (max 20 char)",
  "address": "string (max 100 char)"
}
```
...
### v3.0.0 (25 Mei 2026) — Profile Toko & Identitas SAKA. OMNI
- **[FEAT]** Modul Profile Toko — nama toko, WhatsApp, alamat jalan disimpan di localStorage
- **[FEAT]** Nama toko dinamis di header, judul browser, dan tab Tentang
- **[FEAT]** Identitas toko tercetak di struk/resi (nama, alamat, WhatsApp)
- **[FEAT]** Identitas toko di header CSV laporan penjualan
- **[FEAT]** Footer "powered by SAKA. OMNI - POS UMKM" di aplikasi dan struk
- **[FEAT]** Pencipta diubah ke SAKA. OMNI - POS UMKM di tab Tentang
- **[FEAT]** Preview struk di modal Profile Toko
- **[FEAT]** Tombol Profile Toko di sticky header
- **[UX]** Nama aplikasi diubah dari KasirPOS ke OMNI - POS UMKM
...
Proyek ini dikembangkan oleh **SAKA. OMNI - POS UMKM**. Untuk melakukan modifikasi:
...
*Dibangun oleh SAKA. OMNI - POS UMKM — arsitektur diinspirasi oleh template medis "Jadwal Obat 10 Hari - Siklus Medis 24 Jam"*
