# SKILL.md — OMNI - POS UMKM Universal Agentic Skill Specification

> **Versi Skill:** 2.0.0 | **Kompatibilitas:** Agentic Framework v2+ | **Tipe:** Web Application Skill | **App Version:** 3.0.0
...
OMNI - POS UMKM adalah skill agentic yang menghasilkan aplikasi Point of Sale interaktif berbasis web. Skill ini mampu membuat, mengkonfigurasi, dan meng-deploy sistem kasir lengkap dengan modul transaksi, profile toko, diskon (per item & global), manajemen inventori, dan pelaporan penjualan — semuanya dalam satu file HTML yang berjalan tanpa server.

Skill ini dirancang untuk digunakan oleh AI agent dalam merespons permintaan pengguna yang membutuhkan sistem POS/kasir, dengan kemampuan kustomisasi produk, identitas toko, format struk, konfigurasi shortcut pembayaran, dan sistem diskon yang fleksibel.
...
- "point of sale", "POS interaktif", "POS UMKM"
...
- "profile toko", "identitas toko", "nama toko di struk"
...
- "store profile", "shop identity", "brand on receipt"
...
- Pengguna meminta kustomisasi identitas toko di struk/laporan
...
### Capability 2: Profile Toko
```yaml
id: store-profile
description: Modul identitas toko yang tersinkronisasi ke seluruh aplikasi
inputs:
  - store_name: string (max 50 char, required)
  - store_whatsapp: string (max 20 char, optional)
  - store_address: string (max 100 char, optional)
outputs:
  - dynamic_header: nama toko mengganti default di sticky header
  - dynamic_title: judul tab browser mengikuti nama toko
  - receipt_branding: nama, alamat, WhatsApp tercetak di struk
  - csv_header: nama toko di header file CSV laporan
  - about_section: nama toko di tab Tentang
  - preview: preview struk real-time saat mengisi form
storage:
  type: localStorage
  key: pos_store_profile
  schema: { name: string, whatsapp: string, address: string }
```

### Capability 3: Sistem Diskon
```yaml
id: discount-system
...
### Capability 4: Cetak Struk Termal
```yaml
id: thermal-receipt
description: Cetak struk pembelian untuk printer termal 58mm dengan identitas toko
inputs:
  - paper_width: string (default: "58mm")
  - shop_name: string (dari STORE_PROFILE, default: "OMNI - POS UMKM")
  - shop_address: string (dari STORE_PROFILE)
  - shop_whatsapp: string (dari STORE_PROFILE)
  - footer_message: string (default: "Terima kasih atas kunjungan Anda!")
  - powered_by: string (default: "powered by SAKA. OMNI - POS UMKM")
outputs:
  - receipt_preview: modal preview struk dengan identitas toko
  - print_stylesheet: CSS @media print dengan ukuran termal
  - receipt_data: format monospace Courier New
  - store_identity: nama toko + alamat + WhatsApp di header struk
  ...
  - powered_by_footer: footer pencipta di bagian bawah struk
```

### Capability 5: Manajemen Produk CRUD
```yaml
id: product-crud
...
### Capability 6: Laporan Penjualan
```yaml
id: sales-report
...
inputs:
  ...
  - csv_store_header: boolean (default: true, menyertakan nama toko)
outputs:
  ...
  - csv_download: file CSV dengan header nama toko + data rows
```

### Capability 7: Keyboard Shortcut
```yaml
...
```json
{
  "pos_config": {
    ...
    "store_profile": {
      "storage_key": "pos_store_profile",
      "fields": ["name", "whatsapp", "address"],
      "dynamic_targets": ["header", "title", "about_section", "receipt_preview", "receipt_print", "csv_header"]
    },
    "receipt": {
      "paper_width": "58mm",
      "font_family": "Courier New",
      "shop_name_source": "STORE_PROFILE.name",
      "shop_address_source": "STORE_PROFILE.address",
      "shop_whatsapp_source": "STORE_PROFILE.whatsapp",
      "footer_message": "Terima kasih atas kunjungan Anda!",
      "powered_by": "powered by SAKA. OMNI - POS UMKM",
...
2. PARSE configuration from user context:
   ...
   - Store profile (name, WhatsApp, address)
...
3. GENERATE index.html:
   ...
   - Configure store profile module with localStorage persistence
   - Configure shortcuts, currency, receipt format
   - Apply discount system (per item + global, percent + nominal)
   - Apply store branding to receipt preview + print
   - Apply "powered by SAKA. OMNI - POS UMKM" footer
   ...
   ↓
4. SAVE to /home/z/my-project/download/index.html
   ↓
...
- [x] **Store Profile Sanitization** — Data profile di-escape saat di-render ke struk dan UI
...
|-----------|---------|-------------|
| Nama Toko | "OMNI - POS UMKM" | Ganti via modal Profile Toko, tersimpan di localStorage |
| WhatsApp | (kosong) | Ganti via modal Profile Toko |
| Alamat Jalan | (kosong) | Ganti via modal Profile Toko |
...
| Powered By | "SAKA. OMNI - POS UMKM" | Ganti di footer + struk |
...
```html
<iframe src="index.html" width="100%" height="800"></iframe>
```
...
- [ ] Profile Toko: klik tombol → modal muncul → isi form → simpan → nama berubah di header
- [ ] Profile Toko: nama, alamat, WhatsApp tampil di struk preview dan cetak
- [ ] Profile Toko: data persist setelah refresh halaman
- [ ] Profile Toko: preview struk real-time saat mengetik
...
- [ ] Laporan: Export CSV berfungsi (header menyertakan nama toko)
- [ ] Laporan: Hapus transaksi individual
- [ ] Tab Tentang: pencipta SAKA. OMNI - POS UMKM tercantum
- [ ] Footer "powered by SAKA. OMNI - POS UMKM" tampil di aplikasi
...
### Skill v2.0.0 (25 Mei 2026)
- Update app_version ke 3.0.0
- Tambah Capability 2: Profile Toko (nama, WhatsApp, alamat, dynamic branding)
- Update Capability 4: Struk termasuk identitas toko dan footer powered by
- Update Capability 6: CSV header menyertakan nama toko
- Update Configuration Schema: tambah `store_profile`, `powered_by_footer`, `profile_key`
- Update Testing Checklist: tambah test case profile toko
- Update Customization Points: tambah parameter profile toko
- Ubah developer ke SAKA. OMNI - POS UMKM / mlev@2026
- Ubah nama app dari KasirPOS ke OMNI - POS UMKM
